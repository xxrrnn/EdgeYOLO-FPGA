...
try:
    ...
    from chip_config import DCIM_INT8_OUT_CH_PER_TILE, DCIM_INT16_OUT_CH_PER_TILE
except ImportError:
    DCIM_INT8_OUT_CH_PER_TILE = 16
    DCIM_INT16_OUT_CH_PER_TILE = 8
...
def _is_int16_manifest(run_dir: Path) -> bool:
    mf = run_dir / "manifest.txt"
    if not mf.exists():
        return False
    for line in mf.read_text().splitlines():
        if line.startswith("int16:"):
            return line.split(":", 1)[1].strip() in {"1", "true", "True"}
        if line.startswith("shape:") and "[int16]" in line:
            return True
    return False


def _active_tiles_from_manifest(run_dir: Path) -> int:
    matmul_n = _matmul_n_from_manifest(run_dir)
    ch = DCIM_INT16_OUT_CH_PER_TILE if _is_int16_manifest(run_dir) else DCIM_INT8_OUT_CH_PER_TILE
    return min(DCIM_NUM_TILES, (int(matmul_n) + ch - 1) // ch)
