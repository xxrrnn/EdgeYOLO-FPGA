...
if {[info exists ::env(DCIM_OOC_DIR)] && $::env(DCIM_OOC_DIR) ne ""} {
    ...
    set out_dir [file normalize "$repo_root/test/tops/output/fpga/dcim_stream_ooc"]
}
file mkdir $out_dir

create_project -in_memory -part xcvu37p-fsvh2892-2L-e
set include_dirs [list \
    "$repo_root/project/rtl/chip" \
    "$repo_root/project/rtl/common" \
    "$repo_root/project/rtl/ref/DCIM/src/inc" \
    "$repo_root/project/rtl/ref/DCIM/src/dcim" \
    "$repo_root/project/rtl/ref/DCIM/src/model" \
]
set rtl_files [list \
