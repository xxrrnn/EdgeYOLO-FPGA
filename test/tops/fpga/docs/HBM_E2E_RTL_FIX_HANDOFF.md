...
```powershell
python test/tops/fpga/tools/e2e_isolate.py --dcim-lock
...
PCIe reset 后立刻复测（`tops_ila_hbmfix_260813`，脚本 `test/tops/fpga/tools/e2e_isolate.py`）：
...
```powershell
python test/tops/fpga/tools/e2e_isolate.py --dcim-host-ibuf --dcim-tile 0   # 应 64000/64000
python test/tops/fpga/tools/e2e_isolate.py --dcim-lock                     # 当前 bit 应挂 px211
```
...
已修：`test/network/host/hbm_flow.py` 的 `_preload_rows` 按 **每一行** 的 dst。`ChipRunnerWin.run_case(staging=hbm)` 恢复 `drain_output=True` + `read_check(from_hbm=True)`。

清 IBUF 后官方验收：

```text
python test/tops/fpga/run.py --staging hbm --repeat-count 1 --quiet-xdma
...
Bitstream SHA-256 与 `test/tops/fpga/docs/tops_ila_hbmfix_260813_release.md` 一致：
...
| Release | `test/tops/fpga/docs/tops_ila_hbmfix_260813_release.md` |
| Host | `test/network/bin/xdma_rw.exe`，`test/network/host/xdma_win.py` |
...
| `test/network/host/hbm_flow.py` | `_preload_rows` 按行 dst；禁止按文件名折叠 |
| `test/network/host/xdma_win.py` | C2H 256B 分块；hbm staging **已开 drain** |
| `test/tops/fpga/run.py` | `--staging hbm` 走官方 drain 读 HBM |
| `test/tops/fpga/tools/e2e_isolate.py` | YOLO 第一层 OH tile：`--dcim-lock` / `--dcim-host-ibuf` / im2col |
| `rtl/vpu/INST_Decoder.sv` | CDMA wait：离开 IDLE 才 accept，`seen_busy && ready` 才完成 |
| `test/tops/fpga/tools/cdma_dir_probe.py` | 无 DCIM 的 CDMA 方向/次数探针 |
| `rtl/chip/DCIM_Tile.sv` | 连续 job 状态；`start_pulse` / preload / micro-batch |
| `rtl/chip/DCIM_Partial_Sum_RAM.sv` | 64×512b 累加 RAM，无内容 clear |
| `test/tops/fpga/tools/peak_hbm_retest.py` | 清 IBUF 后 peak load+drain 验收 |
| `project/runtime/hw_runner_win.py` | E2E；权重 H2C→HBM，结果从 VPU_BUF C2H |
...
```powershell
python test/tops/fpga/tools/e2e_isolate.py --dcim-lock          # tile0+tile1 均为 64000/64000
python test/tops/fpga/tools/e2e_isolate.py --dcim-host-ibuf --dcim-tile 0
python test/tops/fpga/tools/e2e_isolate.py --skip-large-cdma --max-conv 1
```
...
2. `run.py`：默认 `--staging preload`；`--staging hbm` 走官方 drain。
