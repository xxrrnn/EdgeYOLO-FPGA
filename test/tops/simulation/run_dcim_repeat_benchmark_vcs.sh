...
RUN_DIR="${RUN_DIR:-$REPO_ROOT/test/tops/output/simulation/repeat_benchmark_vcs}"
...
SOURCES=(
  ...
  "$SCRIPT_DIR/tb_dcim_repeat_benchmark.sv"
)
...
"$VCS" -full64 -j32 -lca -debug_access+all -t ps \
  work.tb_dcim_repeat_benchmark -o simv -l "$RUN_DIR/compile.log"
cd "$RUN_DIR"
"$BUILD_DIR/simv" +FSDB -no_save 2>&1 | tee sim.log
grep -q "BENCHMARK_PASS repeats=4 fires=512 active_cycles=512" sim.log
