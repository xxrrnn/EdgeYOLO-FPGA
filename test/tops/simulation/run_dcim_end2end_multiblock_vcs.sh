...
RUN_DIR="${RUN_DIR:-$REPO_ROOT/test/tops/output/simulation/end2end_multiblock_vcs}"
...
SOURCES=(
  ...
  "$SCRIPT_DIR/tb_dcim_end2end_multiblock.sv"
)
cd "$BUILD_DIR"
...
"$VCS" -full64 -j32 -t ps work.tb_dcim_end2end_multiblock \
  -o simv -l "$RUN_DIR/compile.log"
cd "$RUN_DIR"
"$BUILD_DIR/simv" -no_save 2>&1 | tee sim.log
grep -q "END2END_MULTIBLOCK_PASS pixels=70 acc_depth=3" sim.log
