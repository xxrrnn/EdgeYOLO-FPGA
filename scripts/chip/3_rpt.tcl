...
# 用法: vivado -mode batch -source scripts/chip/3_rpt.tcl
#       或在已打开工程中: source scripts/chip/3_rpt.tcl
...
# 加载最新的 post-route checkpoint（与 2_synth.tcl 命名一致）
set postRouteDcp ""
foreach dcpName {
    post_route.dcp
    post_phys_opt_ap.dcp
    post_place.dcp
} {
    set candidate [file normalize "$ImplOutputDir/$dcpName"]
    if {[file exists $candidate]} {
        set postRouteDcp $candidate
        break
    }
}
if {$postRouteDcp eq ""} {
    error "No implementation checkpoint found under $ImplOutputDir. Run 2_synth.tcl first."
}
...
puts "\[2/7\] Generating detailed failing paths (top 50)..."
