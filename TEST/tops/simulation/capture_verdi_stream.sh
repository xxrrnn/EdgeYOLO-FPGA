...
RUN_DIR="$(cd "$1" && pwd)"
FSDB="$RUN_DIR/dcim_stream_tile.fsdb"
OUTPUT_PNG="${2:-$RUN_DIR/dcim_stream_verdi.png}"
...
if [[ -z "${DISPLAY:-}" ]] || ! /usr/bin/xdpyinfo >/dev/null 2>&1; then
  ...
  if [[ -x /usr/bin/metacity ]]; then
    /usr/bin/metacity --sm-disable >"$RUN_DIR/metacity_stream.log" 2>&1 &
    WM_PID=$!
    sleep 1
  fi
fi

nWave -ssf "$FSDB" -play "$SCRIPT_DIR/verdi_stream.tcl" >"$LOG" 2>&1 &
NWAVE_PID=$!
for _ in $(seq 1 30); do
  grep -q 'STREAM_VERDI_VIEW_READY' "$LOG" 2>/dev/null && break
...
grep -q 'STREAM_VERDI_VIEW_READY' "$LOG" || {
...
echo "STREAM_VERDI_SCREENSHOT=$OUTPUT_PNG"
