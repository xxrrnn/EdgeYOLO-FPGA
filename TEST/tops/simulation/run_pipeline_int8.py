...
LOCAL_OUTPUT = REPO_ROOT / "output" / "tops" / "simulation" / "pipeline_vcs"
...
def capture(server: str, remote_repo: str) -> None:
    run_dir = "output/tops/simulation/pipeline_vcs"
    command = "; ".join(
        (
            "set -e",
...
def fetch(server: str, remote_repo: str, local_output: Path) -> None:
    ...
    remote_run = f"{remote_repo}/output/tops/simulation/pipeline_vcs"
