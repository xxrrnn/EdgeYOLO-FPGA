...
硬件运行仍要求 FPGA 已烧录 TEST/utils/bitstream/ 中的唯一 .bit 文件，并已安装 Xilinx
...
END2END_ROOT = Path(__file__).resolve().parent
TEST_ROOT = END2END_ROOT.parent
...
YOLO_ROOT = END2END_ROOT / "yolo"
RESNET_ROOT = END2END_ROOT / "resnet"
DEFAULT_YOLO_IMG = YOLO_ROOT / "examples" / "000000000139.jpg"
DEFAULT_RESNET_IMG = RESNET_ROOT / "examples" / "n01443537_goldfish.JPEG"
RELEASE_ID = "80832ec_attempt1"
COMPILED_DIR = OUT_DIR / "compiled" / RELEASE_ID
COCO_YOLO_DIR = YOLO_ROOT / "model"
RESNET_MODEL_DIR = RESNET_ROOT / "model"
IMAGENET_MANIFEST = RESNET_ROOT / "examples" / "manifest.json"
COCO_MANIFEST = YOLO_ROOT / "examples" / "manifest.json"
PEAK_TB_DIR = REPO_ROOT / "rtl" / "tb" / "lite_bd" / "module_tb"
BITSTREAM_MANIFEST = UTILS_ROOT / "bitstream" / "manifest.json"
MODEL_INPUTS_MANIFEST = END2END_ROOT / "model_inputs_manifest.json"

ONE_SHOT_DEFAULT_BUILDS = {
    ("resnet", "int8"): COMPILED_DIR / "resnet18_int8",
    ("resnet", "int16"): COMPILED_DIR / "resnet18_int16_widened",
}

ONE_SHOT_COMPILE_PARSED = {
    ("resnet", "int8"): RESNET_MODEL_DIR / "parsed_vai",
    ("resnet", "int16"): RESNET_MODEL_DIR / "parsed_vai_int16_widened",
}
...
def _install_unit_tb_run_module() -> None:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load end-to-end unit runner: {UNIT_TB / 'run.py'}")
...
def _compile_one_shot_artifact(network: str, precision: str, build_dir: Path,
    ...
    cmd = [
        sys.executable, str(COMMON_ROOT / "compiler" / "compile.py"),
...
def _verify_release_bundle(*, require_xdma: bool) -> dict:
    ...
    parsed_sets = [
        ...
        (RESNET_MODEL_DIR / "parsed_vai", 22),
        (RESNET_MODEL_DIR / "parsed_vai_int16_widened", 22),
    ]
    ...
    xdma_files = [UTILS_ROOT / "bin" / name for name in ("xdma_info.exe", "xdma_rw.exe")]
    if require_xdma:
...
def run_vcs_peak(*, mode: str, server: str | None, remote_repo: str | None) -> dict:
    ...
    if server:
        ...
        remote_cmd = (
            ...
            "python3 TEST/end2end/run.py --vcs-only"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--resnet-precision", choices=["int8", "int16", "both"], default="both",
                    help="ResNet18 precision: int8=Vitis AI W8A8 / int16=widened values on INT16/INT64 hardware / both")
...
def main() -> int:
    ...
    resnet_precisions = _expand(resnet_raw, ["int8", "int16"])
