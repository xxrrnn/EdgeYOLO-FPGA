...
END2END_ROOT = _THIS.parent
REPO_ROOT = END2END_ROOT.parents[1]
UNIT_TB_ROOT = END2END_ROOT / "common" / "unit_tb"
sys.path.insert(0, str(_THIS))
sys.path.insert(0, str(UNIT_TB_ROOT))
sys.path.insert(0, str(REPO_ROOT / "TEST" / "utils"))
...
def prepare_vai_int16() -> Path:
    ...
    if not _weights_ready(PARSED_VAI):
        raise FileNotFoundError(
            ...
            "TEST/end2end/resnet/model/parsed_vai from the release checkout."
        )
...
def configure_resnet_precision(precision: str) -> Path:
    if precision in ("int8", "vai", "int8_vai"):
        parsed = PARSED_VAI
        ...
        raise ValueError(f"unsupported ResNet precision: {precision!r}  (use 'int8' or 'int16')")
