...
torch.onnx.export) and produce TEST/end2end/resnet/model/parsed_qdq/ matching the
schema that lower.py + compile.py consume.

Output:
    TEST/end2end/resnet/model/parsed_qdq/
...
def main():
    ...
    ap.add_argument("--onnx", default=None,
                    help="QDQ resnet18 ONNX (default: TEST/end2end/resnet/model/resnet18_w8a8.onnx)")
    ap.add_argument("--output", default=None,
                    help="output dir (default: TEST/end2end/resnet/model/parsed_qdq)")
    args = ap.parse_args()

    repo = Path(__file__).resolve().parents[5]
    model_dir = repo / "TEST" / "end2end" / "resnet" / "model"
    onnx_path = args.onnx or str(model_dir / "resnet18_w8a8.onnx")
    out_dir = args.output or str(model_dir / "parsed_qdq")
