...
END2END_ROOT = _THIS.parents[2]
UTILS_ROOT = REPO_ROOT / "TEST" / "utils"
YOLO_MODEL_ROOT = END2END_ROOT / "yolo" / "model"
RESNET_MODEL_ROOT = END2END_ROOT / "resnet" / "model"
sys.path.insert(0, str(_THIS.parent))
sys.path.insert(0, str(UTILS_ROOT))
sys.path.insert(0, str(END2END_ROOT / "yolo"))
sys.path.insert(0, str(END2END_ROOT / "resnet"))
...
_YOLO_PARSED = YOLO_MODEL_ROOT / "parsed_int8" / "network.json"
...
def preprocess_resnet18(img_rgb: np.ndarray) -> np.ndarray:
    ...
    # 从 parsed weights 获取
    weights_dir = RESNET_MODEL_ROOT / "parsed_vai" / "weights"
...
def run_yolov5n_inference(runner, img_path: str, dry_run: bool):
    ...
    weights_dir = str(YOLO_MODEL_ROOT / "parsed_int8" / "weights")
...
def main():
    ...
    if args.image:
        ...
        # 默认使用验证集
        default_dir = REPO_ROOT / "model" / "algorithm" / "Infrared-Object-Detection" / "datasets" / "infrared" / "images" / "val"
