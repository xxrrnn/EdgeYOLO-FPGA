...
def _resolve_xdma_rw_exe() -> Path:
    ...
    for candidate in (
        REPO_ROOT / "test" / "network" / "bin" / "xdma_rw.exe",
        REPO_ROOT / "test" / "network" / "bin" / "xdma_rw",
    ):
        if candidate.exists():
            return candidate
    return REPO_ROOT / "test" / "network" / "bin" / "xdma_rw.exe"
...
try:
    import sys as _sys
    _tools = REPO_ROOT / "project" / "runtime"
    _host = REPO_ROOT / "test" / "network" / "host"
    for _extra in (_tools, _host):
...
def _xdma_interface_path() -> str | None:
    ...
    info_exe = REPO_ROOT / "test" / "network" / "bin" / "xdma_info.exe"
    if info_exe.is_file():
        try:
            proc = subprocess.run(
    ...
    def __init__(
        ...
        if not self._direct and not Path(self.exe).exists():
            raise FileNotFoundError(
                ...
                "Set XDMA_RW_EXE, or extract test/network/bin.zip so test/network/bin/xdma_rw.exe exists."
            )
    ...
    def upload_hbm(
        ...
        for chk in self._parse_checks_meta(run_dir):
            ...
            if dst_off >= TILE_OBUF_CHK_SENTINEL:
                ...
                from hbm_flow import _active_tiles_from_manifest
                active_tiles = _active_tiles_from_manifest(run_dir)
                stride = DCIM_NUM_TILES * wpt
        ...
        for chk in self._parse_checks_meta(run_dir):
            ...
            if from_hbm:
                if dst_off >= TILE_OBUF_CHK_SENTINEL:
                    ...
                    from hbm_flow import hbm_drain_remap, _active_tiles_from_manifest
                    active_tiles = _active_tiles_from_manifest(run_dir)
            ...
            (run_dir / "fpga_got.bin").write_bytes(bytes(got_raw))
