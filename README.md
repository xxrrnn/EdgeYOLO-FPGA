...
## Stable c1773f6 自包含里程碑

这是当前推荐的可复现版本。`main` 已包含运行所需的 bitstream、XDMA 用户态
工具、三套解析后模型参数、六套冻结的 compiler artifacts、测试图片和参考结果。
在另一台已安装 Windows XDMA 驱动的机器上，不需要另外下载 `.pt`、`.onnx`、
权重或 bitstream。

### 支持范围

| Workload | FPGA 主干 | Host 边界 | 量化模式 |
| --- | --- | --- | --- |
| Infrared YOLOv5n | backbone + FPN/PAN | preprocess、detect head、NMS | INT8 / INT16-widened |
| COCO YOLOv5n | backbone + FPN/PAN | preprocess、detect head、NMS | INT8 / INT16-widened |
| ResNet18 | conv、maxpool、residual add、GAP 前 feature | 图片准备、FC、softmax/top-k | INT8 / INT16-widened |

这里的 `INT16-widened` 是把已验证的 INT8 数值装入 INT16 容器，用于覆盖 FPGA
INT16 数据通路；scale 与 INT8 相同。它不是使用 `[-32768, 32767]` 权重的 true
W16A16。true W16A16/INT64 accumulator 的实验 RTL 不属于本稳定里程碑。

### 1. 获取与安装运行环境

```powershell
git clone <this-repository-url> EdgeYOLO-FPGA
cd EdgeYOLO-FPGA
git checkout main
...
运行端只需要仓库内的解析后参数，不需要 PyTorch、ONNX Runtime、Vivado 或网络
下载。板卡必须已经安装可用的 Xilinx XDMA Windows 驱动；仓库包含
`tests/bin/xdma_info.exe` 和 `tests/bin/xdma_rw.exe`，但不包含内核驱动安装包。

### 2. 部署固定 bitstream

使用 Vivado Hardware Manager 或现有板卡部署流程刷入仓库根目录的
[`chip.bit`](chip.bit)。该文件来自稳定 RTL 基线 `c1773f6`：

```text
size   = 84,989,276 bytes
SHA256 = 314d65d3711d464d60f8f5d158d270275d9cc2203ddbaa3b41c7e6a64823764d
```

校验文件：

```powershell
(Get-FileHash .\chip.bit -Algorithm SHA256).Hash
```

刷写后按板卡流程重新枚举 PCIe/XDMA。先做基础门禁，三条命令都应在数秒内结束，
decoder 状态应为 `0x00000000`（idle）或 `0x00000002`（done）：

```powershell
tests\bin\xdma_info.exe
tests\bin\xdma_rw.exe c2h_0 read 0x0 -l 16
python tests\chip\runtime\hw_runner_win.py --build-dir artifacts\c1773f6\yolo_ir_int8 --status-only --quiet-xdma
```

如果 `xdma_info.exe` 本身就很慢或 H2C/C2H 报错，不要继续运行网络。该现象发生在
网络 program 启动之前，优先检查 bitstream 是否正确、PCIe link、Windows XDMA
驱动枚举以及板卡复位状态。
...
```powershell
# Infrared YOLOv5n
python run.py --one-shot --network yolo --yolo-model infrared --yolo-precision int8 --one-shot-poll-timeout-s 300
python run.py --one-shot --network yolo --yolo-model infrared --yolo-precision int16 --one-shot-poll-timeout-s 300

# COCO/original YOLOv5n
python run.py --one-shot --network yolo --yolo-model coco --yolo-precision int8 --conf 0.25 --one-shot-poll-timeout-s 300
python run.py --one-shot --network yolo --yolo-model coco --yolo-precision int16 --conf 0.25 --one-shot-poll-timeout-s 300
...
`run.py` 默认直接使用 `artifacts/c1773f6/` 中冻结的 program、weights 和 WB，
并在每个 workload 前执行 decoder soft reset，因此六项可以连续运行而不需要整板
reset。默认不会删除已有输出目录；同一图片和 workload 的同名结果会被更新，其他
结果会保留。

一次运行红外 YOLO + ResNet 的四项组合：

```powershell
python run.py --one-shot --network all --yolo-model infrared --yolo-precision both --resnet-precision both --one-shot-poll-timeout-s 300
```

### 4. 结果位置与验收值

| Workload | 输出目录 | 已验证结果 | runner / execute |
| --- | --- | --- | --- |
| Infrared YOLO INT8 | `output/inference/yolo_infrared/one_shot_int8/` | 1 person, conf 0.8108 | 2.501s / 2.112s |
| Infrared YOLO INT16 | `output/inference/yolo_infrared/one_shot_int16/` | 1 person, conf 0.7381 | 3.071s / 2.594s |
| COCO YOLO INT8 | `output/inference/yolo_coco/one_shot_int8/` | 2 chair + 1 tv | 2.607s / 2.120s |
| COCO YOLO INT16 | `output/inference/yolo_coco/one_shot_int16/` | 2 chair + 1 tv | 3.024s / 2.538s |
| ResNet18 INT8 | `output/inference/resnet/one_shot_int8/` | Top-1 goldfish(1) | 2.794s / 2.358s |
| ResNet18 INT16 | `output/inference/resnet/one_shot_int16/` | Top-1 goldfish(1) | 6.896s / 4.054s |

每个目录包含：

- `*.jpg`：YOLO 检测框或 ResNet Top-5 可视化；
- `*.json`：检测框/类别或 Top-5 数值；
- `*_timing.json`：权重上传、program 上传、执行和读回耗时；
- `features/` 与 `*.bin`：本次 FPGA feature，供 data-level compare 使用。

六项连续测试的 FPGA runner 合计为 `20.894s`。YOLO PAN feature 相对 compiler
numpy golden 的最大绝对误差不超过 `1.90735e-06`；ResNet INT8/INT16 最终
feature 均为 `max_abs=0`。冻结的参考图片、JSON、timing 和 feature 位于
`artifacts/c1773f6/reference_results/`，可用于不连接 FPGA 时审查本里程碑结果。

### 5. 仓库内已冻结的部署内容

```text
chip.bit                                  stable c1773f6 bitstream
artifacts/c1773f6/                        six tested program/weights/WB bundles
model/yolov5n/parsed*                     infrared YOLO INT8/INT16-widened
model/yolov5n_coco50k_qat/parsed*         COCO YOLO INT8/INT16-widened
model/resnet18/parsed_vai*                ResNet18 INT8/INT16-widened
test_yolo.jpg                             infrared sample
model/yolov5n_coco50k_qat/template/*.jpg  COCO sample
test_resnet_2.JPEG                        ImageNet sample
tests/bin/*.exe                           Windows XDMA user tools
```

原始 `.pt`/`.onnx` 只用于重新训练或重新导出，不参与上述 FPGA 推理，所以没有放入
milestone。需要重新编译 artifact 时，必须显式指定新的输出目录，避免覆盖冻结结果：

```powershell
python run.py --one-shot --network yolo --yolo-model infrared --yolo-precision int8 --one-shot-recompile --one-shot-build-dir output\recompiled\yolo_ir_int8
```
