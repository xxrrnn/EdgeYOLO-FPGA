...
localparam ACT_HEIGHT = 24;
localparam ACT_BLOCKS = 1152;
localparam ACT_BYTES  = 1152 * 32;


reg [255:0] act [0:1151];

initial begin
    $display("Loading act_data.mem ...");
    $readmemh(`MEM_MP_ACT, act);
//"G:/PKU/Yolo_on_FPGA/Architecture/YOLO rtl/goldenmodel/act_data.mem
    $display("Loaded %0d blocks into act", ACT_BLOCKS);
end


localparam EXPECTED_CHANNEL_NUM = 64;
localparam EXPECTED_HEIGHT = 20;
localparam EXPECTED_BLOCKS = 800;
localparam EXPECTED_BYTES  = 800 * 32;

reg [255:0] expected [0:12799];

initial begin
    $display("Loading expected_data.mem ...");
    $readmemh(`MEM_MP_EXP, expected);
...
wire all_true = &expected_true;
    ...
    localparam RAM_DEPTH =  ACT_BLOCKS * 2;
  ...
  // =======================
    ...
    // config inputs
    reg [ADDR_WIDTH-1:0] vpu_unit_choose;
    reg [ADDR_WIDTH-1:0] su_src_addr;
    reg [ADDR_WIDTH-1:0] su_src_stride_col;
    reg [ADDR_WIDTH-1:0] su_src_stride_row;
  ...
  mp_unit #(
    ...
    .FP_WIDTH(32),
    .MAX_CHANNEL_NUM(MAX_MP_CHANNEL_NUM)
  ) dut (
    .clk(clk),
    .rst_n(rst_n),
...
integer j;
  ...
  initial begin
    ...
    config_valid = 0;
    mp_unit_start = 0;
    vpu_unit_choose = 32'd2;
    su_src_addr = 32'h100;
    src_c = US_ACT_CHANNEL_NUM;
    su_src_stride_col = US_ACT_CHANNEL_NUM;
    su_src_stride_row = US_ACT_CHANNEL_NUM * US_ACT_HEIGHT;
    src_h = US_ACT_HEIGHT;
    src_w = US_ACT_HEIGHT;
    
    dst_addr = US_ACT_BYTES + 32'h200;
    ...
    for (i = 0; i < US_ACT_BLOCKS; i = i + 1) begin
        @(posedge clk);
        bram_addra <= i[clogb2(RAM_DEPTH-1)-1:0] + (su_src_addr >> 5);
        bram_dina  <= us_act[i];
    ...
    */
    ...
    config_valid = 0;
    mp_unit_start = 0;
    vpu_unit_choose = 32'd1;
    su_src_addr = 32'h100;
    src_c = ACT_CHANNEL_NUM;
    su_src_stride_col = ACT_CHANNEL_NUM;
    su_src_stride_row = ACT_CHANNEL_NUM * ACT_HEIGHT;
    src_h = ACT_HEIGHT - 4;
    src_w = ACT_HEIGHT - 4;
    
    dst_addr = ACT_BYTES + 32'h200;
    ...
    for (j = 0; j < EXPECTED_BLOCKS; j = j + 1) begin
        @(posedge clk);
        bram_ena = 1'b1;
        bram_wea = {NB_COL{1'b0}};  // 只读
        bram_addra = (dst_addr >> 5 )+ j;
        repeat(2) @(posedge clk);
        expected_data = expected[j];
        @(posedge clk);
        expected_true[j] = bram_douta == expected_data;
        is_match =  bram_douta == expected_data;
        @(posedge clk);
        bram_ena = 1'b0;
    end

    if (all_true)
      $display("[%0t] MP self-check: PASS (all %0d output blocks match golden).", $time, EXPECTED_BLOCKS);
    else
      $display("[%0t] MP self-check: FAIL (BRAM readback != mp_expected_data.mem).", $time);
    $finish;
