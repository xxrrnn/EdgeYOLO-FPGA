...
if __name__ == "__main__":
	...
	# result.mem: 每行是 CH_OUT个WD3的数据，不需要按C行分组
	# 先解析为 [ROWS, CH_OUT] 个 WD3-bit 数据，然后按C个通道组合成逻辑通道
	result_mat_raw = transfer_weight(result_in_file, result_out_file, WD=WD3, ROWS=-1, COLS=CH_OUT, sign=sign)
	print(f"DEBUG: result_mat_raw[0, :8] = {result_mat_raw[0, :8]}")  # 前8个物理通道
	# 对于INT8/INT16，需要将C个物理通道组合成1个逻辑通道（高位拼接）
	if C > 1:
		num_rows = result_mat_raw.shape[0]
		num_logical_ch = CH_OUT // C
		result_mat = np.zeros((num_rows, num_logical_ch), dtype=object)
		for r in range(num_rows):
			for lc in range(num_logical_ch):
				# 每C个物理通道组成1个逻辑通道，低位在前
				value = 0
				for p in range(C):
					phys_idx = lc * C + p
					phys_val = result_mat_raw[r, phys_idx]
					# 将符号数转为无符号（保持bit pattern）
					if phys_val < 0:
						phys_val += (1 << WD3)
					value += phys_val << (WD3 * p)
				# 转回有符号数（总位宽 WD3*C）
				total_bits = WD3 * C
				if value >= (1 << (total_bits - 1)):
					value -= (1 << total_bits)
				result_mat[r, lc] = value
				if r == 0:  # DEBUG first row
					print(f"DEBUG: r=0, lc={lc}, phys_channels={[result_mat_raw[0, lc*C+p] for p in range(C)]}, combined={value}")
	else:
		result_mat = result_mat_raw
	print("act Matrix:\n", act_mat)
	print("weight Matrix:\n", weight_mat)
