...
//       
//       Creation Date:  Wed Apr  1 21:10:16 2026
//       Version: 	r0p0
//
...
`ifdef POWER_PINS
module rf_sp_hde64x128 (VDDCE, VDDPE, VSSE, q, clk, cen, wen, a, d, ema, emaw, emas,
    gwen, ret1n, wabl, wablm, rawl, rawlm);
`else
module rf_sp_hde64x128 (q, clk, cen, wen, a, d, ema, emaw, emas, gwen, ret1n, wabl,
    wablm, rawl, rawlm);
`endif
 ...
 wire  clk_;
  ...
  assign rawlm_[1] = rawlm[1];
   ...
   `ifdef ARM_FAULT_MODELING
     rf_sp_hde64x128_error_injection u1(.CLK(clk_), .Q_out(q_), .A(a_int), .CEN(cen_int), .GWEN(gwen_int), .WEN(wen_int), .Q_in(q_int));
  `else
...
`endif
`timescale 1ns/1ps
module rf_sp_hde64x128_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [127:0] Q_out;
   input [127:0] Q_in;
   input CLK;
   input [5:0] A;
