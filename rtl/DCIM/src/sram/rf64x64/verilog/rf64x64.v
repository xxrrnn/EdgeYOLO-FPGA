...
//       
//       Creation Date:  Fri Apr  3 17:42:52 2026
//       Version: 	r0p0
//
...
`ifdef POWER_PINS
module rf64x64 (VDDCE, VDDPE, VSSE, q, clk, cen, wen, a, d, ema, emaw, emas, gwen,
    ret1n);
`else
module rf64x64 (q, clk, cen, wen, a, d, ema, emaw, emas, gwen, ret1n);
`endif
 ...
 wire  clk_;
  ...
  assign ret1n_ = ret1n;
   ...
   `ifdef ARM_FAULT_MODELING
     rf64x64_error_injection u1(.CLK(clk_), .Q_out(q_), .A(a_int), .CEN(cen_int), .GWEN(gwen_int), .WEN(wen_int), .Q_in(q_int));
  `else
...
`endif
  ...
  reg NOT_emas, NOT_gwen, NOT_ret1n;
...
`endif
`timescale 1ns/1ps
module rf64x64_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [63:0] Q_out;
   input [63:0] Q_in;
   input CLK;
   input [5:0] A;
