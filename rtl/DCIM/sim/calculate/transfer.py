...
def _parse_row_bitstring(row_bits: str, WD1: int, CH_OUT: int, signed: bool) -> np.ndarray:
	...
	if len(row_bits) != expected_len:
		raise ValueError(f"Row length {len(row_bits)} != expected {expected_len}")
	elems = []
...
if __name__ == "__main__":
	...
	weight_in_file = os.path.join(os.path.dirname(__file__), "weight.mem")
	...
	act_in_file = os.path.join(os.path.dirname(__file__), "simToolUp.mem")
	...
	result_in_file = os.path.join(os.path.dirname(__file__), "simToolDown.mem")
