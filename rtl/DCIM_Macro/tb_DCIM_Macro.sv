...
module tb_DCIM_Macro;
    ...
    localparam MAX_TEST_ROWS = 80;  // 支持更大的累加深度测试
    localparam T = 10;
    
    reg clk, rst_n, start;
    wire done, ready;
    reg [2:0] mode;
    reg [6:0] acc_depth;  // 扩展到 7-bit 以支持 ACC=80
    reg [15:0] num_rows;
    ...
    task compute_golden(input integer rows);
        ...
        reg signed [63:0] sum;  // 使用 64-bit 避免溢出
        reg signed [7:0] w8;
        reg signed [15:0] w16;
        reg signed [7:0] a8;
        reg signed [15:0] a16;
    ...
    // ========================================================================
    // 结果读取和验证（新格式：连续 INT32）
    // ========================================================================
    // INT8：8 个逻辑通道 × 32-bit = 256-bit → 2 个 OBUF 地址
    // INT16：4 个逻辑通道 × 32-bit = 128-bit → 1 个 OBUF 地址
    // ========================================================================
    reg signed [31:0] dut_int32 [0:MAX_TEST_ROWS-1][0:7];  // 最多 8 个逻辑通道
        ...
        begin
            ...
            for (row = 0; row < num_outputs; row++) begin
                for (ch = 0; ch < num_logical_ch; ch++) begin
                    // 新格式：直接从 dut_int32 读取（已经是连续 INT32）
