...
// ============================================================================
// tb_DCIM_Array_Group - 单组测试平台
// ============================================================================
// 测试单个 DCIM_Array_Group（8 Tile + 1 IBUF + 1 OBUF）
// 验证新架构的基本功能正确性
// ============================================================================

module tb_DCIM_Array_Group;

    localparam TILES_PER_GROUP = 8;
    localparam WD1          = 4;
    localparam CH_IN        = 16;
    ...
    localparam ACT_BASE     = TILES_PER_GROUP * CYCLE + 32;
    localparam [BUF_ADDR_WIDTH-1:0] OUT_STRIDE = BUF_ADDR_WIDTH'(14'h0400);  // 1024，避免地址溢出
    ...
    reg [TILES_PER_GROUP*BUF_ADDR_WIDTH-1:0] out_base_addrs;
    ...
    localparam integer MAX_ACT_ROWS    = 1000;
    localparam integer MAX_OUT_GROUPS  = 500;

    reg signed [15:0] activation[0:MAX_ACT_ROWS-1][0:CH_IN-1];
    reg signed [31:0] golden[0:TILES_PER_GROUP-1][0:MAX_OUT_GROUPS-1][0:7];
    reg signed [3:0] weight_nibble[0:TILES_PER_GROUP-1][0:CH_IN-1][0:CH_OUT-1];
    ...
    task pulse_start_and_wait_done(input integer timeout_cycles);
        integer cnt;
        begin
            @(posedge clk);
            start <= 1'b1;
    ...
    initial begin
        ...
        $display("═══════════════════════════════════════════════════════════════");
        $display("  Tiles per Group: %0d", TILES_PER_GROUP);
    ...
    initial begin
        repeat (36000) #100000000;
        $display("GLOBAL TIMEOUT!");
        $finish;
    end

    initial begin
        $fsdbDumpfile("dcim_array_group.fsdb");
        $fsdbDumpvars(1, tb_DCIM_Array_Group);
    end
