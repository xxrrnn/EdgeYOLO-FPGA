...
CASE_DIR="${VERILATOR_PEAK_RUN_DIR:-$REPO_ROOT/output/verilator_peak_int8}"
BUILD_DIR="${VERILATOR_PEAK_BUILD_DIR:-$REPO_ROOT/output/.verilator_peak_build}"
...
if python3 -c 'import numpy' >/dev/null 2>&1; then
  python3 "$REPO_ROOT/rtl/tb/lite_bd/module_tb/golden_module_tb.py" \
  ...
  python.exe "$(wslpath -w "$REPO_ROOT/rtl/tb/lite_bd/module_tb/golden_module_tb.py")" \
    --module dcim_matmul --case peak_int8_all_tiles --verify-words 0 \
...
python3 "$REPO_ROOT/rtl/tb/lite_bd/module_tb/report_peak_int8.py" \
